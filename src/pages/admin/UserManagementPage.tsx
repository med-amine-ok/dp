...
const UserManagementPage: React.FC = () => {
  ...
  const filteredPatients = mockPatients.filter(patient => {
    const nameMatch = patient.nameFr.toLowerCase().includes(searchQuery.toLowerCase()) ||
                      patient.name.includes(searchQuery);
    return nameMatch;
  });

  const filteredDoctors = mockDoctors.filter(doctor => {
    const nameMatch = doctor.nameFr.toLowerCase().includes(searchQuery.toLowerCase()) ||
                  ...
                  <TableBody>
                    {filteredPatients.map((patient) => (
                      <TableRow key={patient.id}>
                        ...
                        <TableCell className="text-muted-foreground">{patient.registrationDate}</TableCell>
                        <TableCell>
                      ...
                      <TableRow key={doctor.id}>
                        <TableCell>
                          <div className="flex items-center gap-3">
                            ...
                            <span className="font-medium">
                              {language === 'ar' ? doctor.name : doctor.nameFr}
                            </span>
                          </div>
                        </TableCell>
